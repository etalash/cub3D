...
/*   Updated: 2025/04/03 07:14:56 by maba             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		return (printf("ERROR : Map player is invalid\n"), free_sub_map(map),
// 			1);
// 	if (flood_fill(map, data.map->p_x, data.map->p_y, data.map->m_h,
// 			map_wide) != 0)
...
int	valid_map(t_data data)
{
	...
	if (!cheak_player(data, map_info.map_height, map_info.map_width, map))
		return (printf("ERROR:Map player is invlid\n"), free_sub_map(map), 1);
