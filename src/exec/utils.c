...
/*   Updated: 2025/03/19 13:02:33 by stalash          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double calc_wall_height(t_data *data)
{
    ...
    wall_height = (TILE_SIZE / data->ray->perpWallDist) * (data->map->res_w / 2);
    return (wall_height);
}
...
mlx_texture_t *set_texture(t_data *data)
{
    if (data->ray->flag == 0)
    {
        if (data->ray->rayAngle > M_PI / 2 && data->ray->rayAngle < 3 * (M_PI / 2))
            return (data->east);  // East texture
        else
            return (data->west);  // West texture
    }
    else
    {
        if (data->ray->rayAngle > 0 && data->ray->rayAngle < M_PI)
            return (data->south);  // South texture
        else
            return (data->north);  // North texture
    }
}
