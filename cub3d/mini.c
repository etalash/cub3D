...
// /*                                                +#+#+#+#+#+   +#+           */
// /*   Created: 2024/12/28 13:58:04 by stalash           #+#    #+#             */
// /*   Updated: 2024/12/28 14:42:46 by stalash          ###   ########.fr       */
// /*                                                                            */
// /* ************************************************************************** */
...
// {
// 	data.player->x_p = data.map->x_p * 30 + 30 / 2; // player x position in pixels in the center of the tile
// 	data.player->y_p = data.map->y_p * 30 + 30 / 2; // player y position in pixels in the center of the tile
...
// // {
// // 	data->player->x_p = data->map->x_p 
// // }

// void    intit_data(t_map *map)
// {
//     t_data data;
...
//     init_player(data);
// 	mlx_loop_hook(data.map, &game_loop, &mlx); // game loop continuously call a specified function to update the game state and render the frames.
...
// char *intit_map(t_map *map)
// {
//     map = (char *)calloc(1, sizeof(t_map));
//     if (!map)
//         return (NULL);
//     map->map = (char **)calloc(10, sizeof(char *));
//     if (!map->map)
//         return (NULL);
//     map->map[0] = strdup("1111111111111111111111111");
// 	map->map[1] = strdup("1000000000000000000100001");
// 	map->map[2] = strdup("1001000000000000000000001");
// 	map->map[3] = strdup("100100000P000000001000001");
...
# include <stdio.h>
# include <math.h>
# include "lib/MLX42/include/MLX42/MLX42.h" // include the mlx library
...
void	rotate_player(t_mlx *mlx, int i)	// rotate the player
{
	if (i == 1)
	{
		...
		if (mlx->ply->angle > 2 * 3.14159)
			mlx->ply->angle -= 2 * 3.14159;
	}
	else
	{
...
int	get_color(t_mlx *mlx, int flag)	// get the color of the wall
{
	...
	if (flag == 0)
	{
		if (mlx->ray->ray_ngl > 3.14159 / 2 && mlx->ray->ray_ngl < 3 * (3.14159 / 2))
	...
	{
		if (mlx->ray->ray_ngl > 0 && mlx->ray->ray_ngl < 3.14159)
...
int	unit_circle(float angle, char c)	// check the unit circle
{
	...
	{
		if (angle > (3.14159 / 2) && angle < (3 * 3.14159) / 2)
			return (1);
	}
	return (0);
}
...
void init_the_player(t_mlx mlx)	// init the player structure
{
	...
	mlx.ply->fov_rd = (FOV * 3.14159) / 180; // field of view in radians
	mlx.ply->angle = 3.14159; // player angle
	//the rest of the variables are initialized to zero by calloc
}
